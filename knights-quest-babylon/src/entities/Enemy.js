...
export class Enemy {
  constructor(scene, position, player) {
    ...
    // Magazine system for gun enemies
  ...
  update(deltaTime, game) {
    ...
    // Handle reloading for gun enemies
    if (this.isReloading) {
    ...
    if (this.chasingPlayer) {
      ...
      if (this.hasGun && distToPlayer > 5 && distToPlayer < 35 && this.shootCooldown <= 0 && !this.isReloading) {
        // Check if magazine is empty and needs reload
