...
export class World {
  ...
  _createGround(levelIndex) {
    ...
    mat.emissiveColor = new BABYLON.Color3(c.r * 0.4, c.g * 0.4, c.b * 0.4);
  ...
  _buildCastleMap() {
    ...
    // Additional walls and obstacles for chest hunting
    // Perimeter stone walls
    this._createWall(-25, 0, -15, 8, 3, 1);
    this._createWall(25, 0, -15, 8, 3, 1);
    this._createWall(-20, 0, 20, 10, 3, 1);
    this._createWall(20, 0, 20, 10, 3, 1);

    // Corner guard towers (smaller)
    [[-22, -28], [22, -28], [-25, 30], [25, 30]].forEach(([x, z]) => {
      const tower = this._track(BABYLON.MeshBuilder.CreateCylinder('guardTower', {
        height: 6, diameter: 2.5
      }, this.scene));
      tower.position = new BABYLON.Vector3(x, 3, z);
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const crate = this._track(BABYLON.MeshBuilder.CreateBox('crate', {
        width: 1.2, height: 1.2, depth: 1.2
      }, this.scene));
      crate.position = new BABYLON.Vector3(x, 0.6, z);
      crate.rotation.y = Math.random() * Math.PI;
      crate.material = this.materials.hut;
      crate.physicsImpostor = new BABYLON.PhysicsImpostor(
        crate, BABYLON.PhysicsImpostor.BoxImpostor,
        { mass: 0, friction: 0.5 }, this.scene
      );
    }
  }
  ...
  _buildForestMap() {
    ...
    // Dense thickets (impassable tree clusters)
    for (let i = 0; i < 8; i++) {
      const cx = -50 + Math.random() * 100;
      const cz = -50 + Math.random() * 100;
      for (let j = 0; j < 4; j++) {
        const ox = cx + (Math.random() - 0.5) * 3;
        const oz = cz + (Math.random() - 0.5) * 3;
        this._createTree(ox, oz, 3 + Math.random() * 2);
      }
    }
  }
  ...
  _buildSkyMap() {
    ...
    // Floating debris/platforms between islands
    for (let i = 0; i < 10; i++) {
      const x = -40 + Math.random() * 80;
      const y = 8 + Math.random() * 10;
      const z = -50 + Math.random() * 80;
      const debris = this._track(BABYLON.MeshBuilder.CreateBox('debris', {
        width: 2 + Math.random() * 2,
        height: 0.5,
        depth: 2 + Math.random() * 2
      }, this.scene));
      debris.position = new BABYLON.Vector3(x, y, z);
      debris.rotation.y = Math.random() * Math.PI;
      debris.material = this.materials.stone;
      debris.physicsImpostor = new BABYLON.PhysicsImpostor(
        debris, BABYLON.PhysicsImpostor.BoxImpostor,
        { mass: 0, friction: 0.8 }, this.scene
      );
    }

    // Stone pillars on main islands
    [[0, 8, 2], [25, 10, -15], [-30, 6, 20]].forEach(([x, y, z]) => {
      const pillar = this._track(BABYLON.MeshBuilder.CreateCylinder('skyPillar', {
        height: 6, diameter: 1.5
      }, this.scene));
      pillar.position = new BABYLON.Vector3(x, y + 3, z);
  ...
  _buildCastle(x, z) {
    ...
    floorMat.emissiveColor = new BABYLON.Color3(0.16, 0.16, 0.16);
